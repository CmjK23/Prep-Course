...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  digito = parseInt(n.toString()[0])
  if (digito === 9){return true;}
  else {return false;}
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  b = true
  for (let i = 1; i < arreglo.length && b; i++){
    if (arreglo[i] !== arreglo[i-1]){b = false;}
  }
  return b;
} 
...
function mayorACien(array) {
  ...
  var new_array = [];
  for (let i = 0; i < array.length; i++){
    if (array[i] > 100){new_array.push(array[i]);}
  }
  return new_array;
}
